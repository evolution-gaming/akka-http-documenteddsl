...
class PathDDirectivesSpec extends AnyWordSpec with DDirectivesSpec with ScalatestRouteTest {
  ...
  "PathPrefix" must {
    ...
    "be applied for N segments" in {
      PathPrefix("a" / "b" / "c").describe(RouteDocumentation()).path mustBe
        PathDocumentation.NonEmpty(prefix = Some("a/b/c"))
    }
    "mix by conjunction" in {
      (PathPrefix("foo") & Path("bar")).describe(RouteDocumentation()).path mustBe
        PathDocumentation.NonEmpty(prefix = Some("foo"), path = Some("bar"))
    }
    "mix by apply" in {
  ...
  "PathSuffix" must {
    ...
    "be applied for N segments" in {
      PathSuffix("a" / "b" / "c").describe(RouteDocumentation()).path mustBe
        PathDocumentation.NonEmpty(suffix = Some("a/b/c"))
    }
    "mix by conjunction" in {
      (Path("bar") & PathSuffix("baz")).describe(RouteDocumentation()).path mustBe
        PathDocumentation.NonEmpty(path = Some("bar"), suffix = Some("baz"))
      (PathPrefix("foo") & Path("bar") & PathSuffix("baz")).describe(RouteDocumentation()).path mustBe
        PathDocumentation.NonEmpty(prefix = Some("foo"), path = Some("bar"), suffix = Some("baz"))
    }
    "mix by apply" in {
      val route = Path("bar") {
    ...
    "mix by apply (N inner routes)" in {
      ...
      doc.tail.head.path mustBe
        PathDocumentation.NonEmpty(prefix = Some("foo"), path = Some("bar"), suffix = Some("bax"))
    }
  }
  ...
  "Path" must {
    ...
    "be applied for N segments with string variables" in {
      ...
      doc.parameters mustBe Some(List(ParamDocumentation(
        "user id",
        Json.obj("type" -> "string"),
        required = true,
        ParamDocumentation.Origin.Path,
      )))
    }
    ...
    "be applied for N segments with date variables" in {
      ...
      doc.parameters mustBe Some(List(ParamDocumentation(
        "creation date",
        Json.obj("type" -> "string", "format" -> "date", "pattern" -> "^[0-9]{4}-[0-9]{2}-[0-9]{2}$"),
        required = true,
        ParamDocumentation.Origin.Path,
      )))
    }
    "be applied for N segments with RegExp variables" in {
      val doc = Path(
        "a" / Segment.re("^[a-z0-9_\\.-]+@[\\da-z\\.-]+\\.[a-z\\.]{2,6}$".r, "email") / "c",
      ).describe(RouteDocumentation())
      doc.path mustBe PathDocumentation.NonEmpty(path = Some("a/{string}/c"))
      doc.parameters mustBe Some(List(ParamDocumentation(
        "email",
        Json.obj(
    ...
    "be translated to akka (N segments with RegExp variables)" in {
      val route = Path("a" / Segment.re("^[a-z0-9_\\.-]+@[\\da-z\\.-]+\\.[a-z\\.]{2,6}$".r) / "c") apply { x =>
        complete(s"$x")
      }
