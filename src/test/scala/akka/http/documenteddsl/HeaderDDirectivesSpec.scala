...
class HeaderDDirectivesSpec extends AnyWordSpec with DDirectivesSpec with ScalatestRouteTest {
  ...
  "Header" must {
    "be applied to route documentation" in {
      ...
      Header(Symbol("CustomHeader"), "A", "B").describe(RouteDocumentation()).headers mustBe
        Some(List(HeaderDocumentation(
          name = "CustomHeader",
          required = true,
          constraints = Some(Set("A", "B")),
        )))
  ...
  "OptHeader" must {
    "be applied to route documentation" in {
      ...
      OptHeader("-x-custom-header", "A", "B").describe(RouteDocumentation()).headers mustBe
        Some(List(HeaderDocumentation(
          name = "-x-custom-header",
          required = false,
          constraints = Some(Set("A", "B")),
      ...
      OptHeader(Symbol("CustomHeader"), "A", "B").describe(RouteDocumentation()).headers mustBe
        Some(List(HeaderDocumentation(
          name = "CustomHeader",
          required = false,
          constraints = Some(Set("A", "B")),
