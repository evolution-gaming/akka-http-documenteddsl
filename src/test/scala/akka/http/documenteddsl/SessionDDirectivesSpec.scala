...
class SessionDDirectivesSpec extends AnyWordSpec with DDirectivesSpec with ScalatestRouteTest {
  ...
  "Session" must {
    ...
    "be handled. correct cookie and proper permissions - correct session" in
      new CorrectCookieProperPerms with CustomScope
    ...
    "be handled (generic impl). correct cookie but lack of permissions - no session" in
      new CorrectCookieLackOfPerms with GenericScope
    "be handled (generic impl). correct cookie - correct session" in new CorrectCookie with GenericScope
    "be handled (generic impl). correct cookie and proper permissions - correct session" in
      new CorrectCookieProperPerms with GenericScope
    ...
    trait CorrectCookieLackOfPerms { this: Scope =>
      val route = Session[UserSession, Perm](Write) apply { session =>
        complete(s"${ session.id }, ${ session.userId }")
      }
      Get("/") ~> addHeader(Cookie("session-id", "my-session-id")) ~> route ~> check {
        handled mustBe false
    ...
    trait CorrectCookieProperPerms { this: Scope =>
      val route = Session[UserSession, Perm](Read, Write) apply { session =>
        complete(s"${ session.id }, ${ session.userId }")
      }
      Get("/") ~> addHeader(Cookie("session-id", "my-session-id")) ~> route ~> check {
        handled mustBe true
    ...
    def authorize(sessionId: String): Set[Perm] => Option[UserSession] = {
      permissions =>
        sessionStore get sessionId filter
          (session => permissions.isEmpty || permissions.intersect(session.role.permissions).nonEmpty)
    }
    ...
    implicit lazy val sessionProvider: SessionProvider[UserSession, Perm] = new SessionProvider[UserSession, Perm] {
      ...
      override def obtain(permissions: Set[Perm]): Directive1[UserSession] = {
        cookie("session-id") flatMap { cp =>
          (sessionStore get cp.value).fold[Directive1[UserSession]](reject(AuthorizationFailedRejection.get)) {
            session =>
