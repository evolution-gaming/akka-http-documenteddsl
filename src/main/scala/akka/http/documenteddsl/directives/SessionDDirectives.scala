...
trait SessionDDirectives {
  ...
  case class Session[T, P](
    permissions: P*,
  )(implicit
    sp: SessionProvider[T, P],
    permissionCodeExtractor: PermissionCodeExtractor[P] = (x: P) => x.toString,
  ) extends DDirective1[T] {
    def describe(
      w: RouteDocumentation,
    )(implicit
      as: AutoSchema,
    ): RouteDocumentation = w.authorized(
    ...
    def flatten(
      implicit
      as: AutoSchema,
    ): DDirective0 = new DDirectiveDelegate(delegate flatMap { _ => Directive.Empty }, describe)
  }

  object Session {
    def apply[T, P: PermissionCodeExtractor](
      implicit
      sp: SessionProvider[T, P],
...
object SessionDDirectives extends SessionDDirectives {
  ...
  class GenericSessionProvider[T, P](cookieName: String, backend: String => Set[P] => Option[T])
  extends SessionProvider[T, P] {
