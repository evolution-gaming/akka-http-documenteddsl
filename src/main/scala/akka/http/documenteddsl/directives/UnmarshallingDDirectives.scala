...
trait UnmarshallingDDirectives {
  ...
  final class SuccessfulOut[T: ru.TypeTag](status: StatusCode, example: Option[JsValue]) extends DDirective0 {
    def describe(
      ...
      as: AutoSchema,
    ): RouteDocumentation = w.outSuccess[T](status.intValue, example)
    def delegate = Directive.Empty
  }

  final class ErrorOut(status: StatusCode, contentType: Option[String], description: Option[String])
  extends DDirective0 {
    def describe(
      w: RouteDocumentation,
    )(implicit
      as: AutoSchema,
    ): RouteDocumentation = w.outError(status, contentType, description)
    def delegate = Directive.Empty
  }
  ...
  object Out {
    ...
    def apply[T: ru.TypeTag](
      example: T,
    )(implicit
      writes: Writes[T],
    ): SuccessfulOut[T] = new SuccessfulOut(StatusCodes.OK, Some(writes writes example))
    def apply[T: ru.TypeTag](
      status: StatusCode,
      example: T,
    )(implicit
      writes: Writes[T],
    ): SuccessfulOut[T] = new SuccessfulOut(status, Some(writes writes example))
    ...
    def apply(status: StatusCode, contentType: ContentType, description: String): ErrorOut =
      new ErrorOut(status, Some(contentType.toString()), Some(description))
  }
